...
const GET_LOGIN_URL = 'https://raw.githubusercontent.com/DEYVIDYT/CineStream-Pro/refs/heads/main/credentials_base64.txt';
...
export class XtremecodesApi {
  ...
  private async initializeCredentials() {
    ...
    try {
        if (isAndroid) {
            ...
            // In a browser, fetch from the GitHub raw content
            ...
            try {
                ...
                console.error("Falha ao analisar o JSON decodificado:", jsonError);
                console.error("Corpo da resposta decodificada:", decodedData);
                throw new Error("A resposta do servidor de credenciais não é um JSON válido após decodificação.");
            }

            if (!Array.isArray(data) || data.length === 0) {
                throw new Error("Nenhum dado de credencial válido encontrado.");
            }

            // Select a random set of credentials
            const randomIndex = Math.floor(Math.random() * data.length);
            const rawCredsData = data[randomIndex];
  ...
  private async makeApiRequest<T>(action: string, params: Record<string, string> = {}): Promise<T> {
    ...
    if (isAndroid) {
        ...
        // In a browser, directly use the Xtremecodes API
        ...
        if (!response.ok) {
          throw new Error(`Falha na requisição à API: ${response.status} ${response.statusText}`);
        }
        ...
        try {
            ...
            console.error("Falha ao analisar o JSON da resposta da API:", text);
  ...
  // Método otimizado para buscar todas as categorias em paralelo
  ...
  // Método otimizado para buscar todos os streams em paralelo
  async getAllStreams(): Promise<{
    liveStreams: Stream[];
    vodStreams: Stream[];
    series: Stream[];
  }> {
    ...
    return { liveStreams, vodStreams, series };
  }

  // Método otimizado para buscar tudo de uma vez
  async getAllData(): Promise<{
    ...
    const [categoriesResult, streamsResult] = await Promise.all([
