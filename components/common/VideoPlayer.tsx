...
import SecondaryVideoPlayer from './SecondaryVideoPlayer';
...
const ClapprVideoPlayer: React.FC<VideoPlayerProps> = ({ src, className }) => {
...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ src, className }) => {
  const { useSecondaryPlayer } = usePlayer();
  
  // Se o player secundário estiver ativado, use o SecondaryVideoPlayer
  if (useSecondaryPlayer) {
    return <SecondaryVideoPlayer src={src} className={className} />;
  }
  
  // Caso contrário, use o player Clappr original
  return <ClapprVideoPlayer src={src} className={className} />;
};
